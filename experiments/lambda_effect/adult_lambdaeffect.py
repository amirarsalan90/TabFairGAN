...
def multiple_runs(num_trainings, df, epochs, batchsize, fair_epochs, lamda, outFile):
    ...
    for i in range(num_trainings):
        if i == 0:
            first_line = "num_trainings: %d, num_epochs: %d, batchsize: %d, fair_epochs:%d, kir" % (num_trainings, epochs, batchsize, fair_epochs)
            print(first_line)
            print2file(first_line, outFile)
            #print2file(buf, outFile)

            second_line = "lamda ,accuracy_original, accuracy_generated, difference_accuracy, f1_original, f1_generated, difference_f1, demographic_parity_data, demographic_parity_classifier"
            print(second_line)
            print2file(second_line, outFile)
        for j in lamda:
            generator, critic, ohe, scaler, data_train, data_test, input_dim = train_plot(df=df, epochs=epochs, batchsize=batchsize, fair_epochs=fair_epochs, lamda=j)
            ...
            df_generated = generator(torch.randn(size=(32561, 110), device=device)).cpu().detach().numpy()
            ...
            buf = '%f, %f, %f , %f, %f, %f, %f, %f, %f' % (j, accuracy_original, accuracy_generated, difference_accuracy, f1_original, f1_generated, difference_f1, demographic_parity_data, demographic_parity_classifier)
            print(buf)
            print2file(buf, outFile)
...
outfile = "results.log"
multiple_runs(num_trainings=5, df=df, epochs=args.num_epochs, batchsize=args.batch_size, fair_epochs=args.num_fair_epochs, lamda=[0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7], outFile=outfile)
